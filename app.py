...
try:
    ...
    class DummyProtobuf:
        ...
        def SerializeToString(self):
            return b''
        def __str__(self):
            return "Dummy protobuf message"
    
    class Garena_420(DummyProtobuf):
        pass
    
    class GameData(DummyProtobuf):
        pass
    
    output_pb2 = type('output_pb2', (), {'Garena_420': Garena_420})()
    my_pb2 = type('my_pb2', (), {'GameData': GameData})()
...
def qR9sT(uid, password):
    ...
    if uid_str in TOKEN_CACHE:
        cached = TOKEN_CACHE[uid_str]
        if time.time() - cached["timestamp"] < 3600:
            return cached["access_token"], cached["open_id"]
    ...
    headers = {
        ...
        'X-Forwarded-For': f"{int(time.time()) % 255}.{int(time.time() * 2) % 255}.{int(time.time() * 3) % 255}.{int(time.time() * 4) % 255}"
    }
    try:
        time.sleep(1)
...
def uV1wX(access_token, open_id):
    ...
    headers = {
        ...
        'ReleaseVersion': cV6xW,
        'X-Forwarded-For': f"{int(time.time() * 5) % 255}.{int(time.time() * 6) % 255}.{int(time.time() * 7) % 255}.{int(time.time() * 8) % 255}"
    }
